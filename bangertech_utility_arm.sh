  ...
  if whiptail --backtitle "The BangerTECH Utility ARM VERSION" --title "CONFIRMATION" --yesno "You are about to install: $CHOICES" 8 82; then 
    ...
    case "$CHOICE" in
      ...
      '"Homebridge"')
        ...
        sudo docker-compose up -d
        whiptail --backtitle "The BangerTECH Utility ARM VERSION" --title "Homebridge" --msgbox "Setup the HomeKit Bridge here http://yourIP:8321" 8 82
      ;;
      '"Grafana"')
        sudo mkdir -p $HOME/docker-compose-data && cd $HOME/docker-compose-data
      ...
      '"CodeServer"')
        ...
        sudo docker-compose up -d
        whiptail --backtitle "The BangerTECH Utility ARM VERSION" --title "CodeServer" --msgbox "Your CodeServer is located here http://yourIP:8443\nPassword is: admin\nYou may change it here $HOME/docker-compose-data/codeserver/docker-compose.yml" 14 82
      ;;
      '"node-exporter"')
        sudo mkdir -p $HOME/docker-compose-data && cd $HOME/docker-compose-data
